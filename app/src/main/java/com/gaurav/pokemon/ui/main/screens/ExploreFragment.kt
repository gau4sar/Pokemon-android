...
class ExploreFragment : Fragment(R.layout.fragment_explore) {
    ...
    private fun getCurrentLocation() {
        if (!requireActivity().isGpsEnabled()) {
            ...
            if (location != null) {
                mainViewModel.moveToLocation.postValue(
                    LatLng(
                        location.latitude,
                        location.longitude
                    )
    ...
    fun generateRandomCoordinates(min: Int, max: Int): LatLng {
        ...
        val currentLong: Double = mainViewModel.mCurrentLocation.value!!.longitude
        val currentLat: Double = mainViewModel.mCurrentLocation.value!!.latitude
